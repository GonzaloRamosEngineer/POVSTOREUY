...
| PF-07 | P1 | `mp-preference` no valida estado de orden | ✅ Cerrado 2026-05-23 | Guard agregado en [mp-preference/route.ts:74-84](src/app/api/mp-preference/route.ts#L74-L84) post-load de orden. Rechaza `409` si `payment_status ∈ {completed, refunded}` o `order_status === 'cancelled'`. Permite `failed` (retry intencional). Mensajes en `apiErrorMessages.mpPreference.orderAlreadyPaid/orderCancelled/orderRefunded`. **Verificado en prod 2026-05-23**: smoke 2/2 contra `https://povstore.uy/api/mp-preference` con POV-889886 (completed) → `409 "ya está pagada"` y POV-356707 (cancelled) → `409 "está cancelada"`. Rama `refunded` no tiene data en prod para testear (sin órdenes refunded), cubierta por código. **Tests del guard agregados 2026-05-24**: [src/app/api/mp-preference/route.test.ts](src/app/api/mp-preference/route.test.ts) con 5 casos (completed/refunded/cancelled → 409; failed → 200 retry intencional; pending → 200 happy path). Suite 132/132. |
...
| PF-09 | P2 | Detección pickup vía `!shipping_address` | ✅ Cerrado 2026-05-23 | ENUM `delivery_method('delivery','pickup')` + columna NOT NULL en `orders` + RPC `create_order_transactional` con `p_delivery_method` ([migrations/20260523_pf09](migrations/20260523_pf09_orders_delivery_method.sql)). Helper [src/lib/orders/deliveryMethod.ts](src/lib/orders/deliveryMethod.ts) (`isPickup`, `isDelivery`, `PICKUP_ADDRESS`) reemplaza el patrón viejo en 9 sitios. Backfill basado en notes pattern (primario) + shipping_address vacío (fallback): 17 pickup + 4 delivery clasificadas, 0 NULLs, 0 false positives. Suite 115/115 + smoke local 3/3. **Verificado en prod 2026-05-23**: smoke 4/4 contra `povstore.uy` post-deploy (home 200, rate-limit intacto 201, order-details con nueva SELECT 404 legítimo, create-order con handler que pasa `p_delivery_method` 400 legítimo). |
...
**Estado consolidado al 2026-05-23:** **10/10 cerrados** (los tres P0 de auth/firma + PF-10 + PF-07 + PF-06 + PF-04 + PF-05 + PF-08 + PF-09).
...
~~**Tests del guard PF-07**~~ — Cerrado 2026-05-24. Ver fila PF-07 arriba.

**Audit cerrado al 100%** 2026-05-23 con PF-09. Los follow-ups arriba son mejoras opcionales, no items del audit.
...
- ~~**Race condition en stock**~~ **CERRADO 2026-05-23** (ver "Stock apply estricto" en Convenciones). `apply_order_stock_once` ahora lockea con `SELECT FOR UPDATE OF p` los products afectados, valida `stock_count >= qty`, y `RAISE EXCEPTION 'insufficient_stock'` si falla. Migration: [migrations/20260523_apply_order_stock_strict.sql](migrations/20260523_apply_order_stock_strict.sql). Quedan **dos limitaciones conscientes documentadas como follow-ups**: (1) el pre-check en [create-order/route.ts](src/app/api/create-order/route.ts) sigue siendo no-transaccional — sólo afecta UX (orden se crea aunque luego falle apply); (2) el UPDATE de `payment_status='completed'` en mp-webhook y admin/orders[id] sigue ocurriendo ANTES del apply, así que si apply falla por insufficient_stock la orden queda en estado inconsistente (`payment_status=completed` + `stock_applied_at=null`) — admin debe reconciliar.
...
**Lado cliente:** preferí [src/lib/api/adminFetch.ts](src/lib/api/adminFetch.ts) (ver sección dedicada abajo para uso). Toda la suite de admin client-side (`OrderHistorySection`, `OrderDetailsModal`, `ProductForm`, `InventoryPageInteractive`) ya usa este wrapper — no quedan callers con `fetch` manual + `getAuthHeader()` ad-hoc.
...
Toda llamada client-side a `/api/admin/*` debe usar [src/lib/api/adminFetch.ts](src/lib/api/adminFetch.ts). Inyecta `Authorization: Bearer ${session.access_token}` automáticamente y lanza `AdminFetchError` (con `.status` y `.body`) si la respuesta no es 2xx. **En uso en toda la suite admin client-side** (`OrderHistorySection`, `OrderDetailsModal`, `ProductForm`, `InventoryPageInteractive`) — la migración del `fetch` manual + `getAuthHeader()` ad-hoc se completó 2026-05-23. Si agregás una llamada nueva client→`/api/admin/*`, usá `adminFetch<T>()` con tipo de respuesta y catch de `AdminFetchError` (que ya expone `.status` y `.body`).
...
### Stock-once
Toda escritura que descuente stock pasa por [src/lib/stock/applyOrderStockOnce.ts](src/lib/stock/applyOrderStockOnce.ts), que invoca la RPC `apply_order_stock_once`. Nunca actualizar `products.stock_count` directamente desde una route.

**Stock apply estricto (post-2026-05-23):** la RPC hace ahora:
1. `SELECT FOR UPDATE` del row de `orders` (idempotencia + lock de orden).
2. `PERFORM 1 FROM products ... FOR UPDATE OF p` sobre **todos los products afectados** — esto serializa cualquier otra transacción que toque los mismos products.
3. Busca primer shortfall (stock_count < qty) en estado ya lockeado. Si existe → `RAISE EXCEPTION 'insufficient_stock'` con `ERRCODE='P0001'` + `DETAIL='product_id=X name=Y available=N needed=M'`.
4. Si pasa validación, decrementa estricto (sin `GREATEST(0, …)` que silenciaba oversold).

El wrapper TS detecta el RAISE EXCEPTION parseando el DETAIL y devuelve `{ ok:false, reason:'insufficient_stock', shortfall:{ productId, name, available, needed } }`. Callers actúan distinto:
- **mp-webhook**: insufficient_stock → **200** con reason en body + log warn estructurado (NO retry — el stock no se arregla en segundos). Otros errores siguen 500 (retry vale la pena).
- **admin/orders/[id] manual mark-paid**: insufficient_stock → **409** con `shortfall` en body para que el admin vea el producto que falta. Otros errores → 500.

**Limitación conocida:** el UPDATE de `payment_status='completed'` ocurre ANTES del apply en ambos callers. Si apply falla por insufficient_stock, la orden queda `payment_status=completed` + `stock_applied_at=null`. La aplicación tiene un path de recovery vía `mustRecoverStock` en webhook (reintenta apply cuando webhook fire de nuevo con misma status) pero admin debe reconciliar manualmente (reponer stock + re-trigger PATCH, o revertir payment_status). Mover el apply ANTES del UPDATE es el follow-up natural si esto pasa con frecuencia.
...
3. **🟡 Quick-action "marcar pagado" inline + export CSV** — *Esfuerzo: ~3-4 h. Impacto: alto operacional.*
...
4. **🟡 Limpieza one-shot del JSONB `packs[].stock` zombie** — *Esfuerzo: ~5 min en Supabase Studio.*
...
- 2026-05-23: **migración a adminFetch completada** (cierra deuda 🟡 de Próximos pasos). 3 componentes restantes (`OrderDetailsModal`, `ProductForm`, `InventoryPageInteractive`) refactoreados de `fetch` manual + `getAuthHeader()` ad-hoc al wrapper canónico `adminFetch<T>()`. 8 call sites migrados (5 en OrderDetailsModal + 2 en ProductForm + 1 en InventoryPageInteractive). Helper local `getAuthHeader()` de OrderDetailsModal y `getAccessToken()` de InventoryPageInteractive eliminados — el wrapper centraliza la inyección del Bearer + parsing de JSON + lanzamiento de `AdminFetchError` con `.status`+`.body`. Manejo de errores uniforme: catch detecta `AdminFetchError` y propaga `err.message` a toasts (que ya leían `err.error` del body, ahora vienen del campo `message` de la clase). Suite 127/127 sin tocar tests (las firmas de respuesta no cambian). Type-check sin errores nuevos. Smoke local 4/4 (home + admin-dashboard + inventory list + inventory new — todos render OK). **Verificado en prod 2026-05-23**: smoke 5/5 contra `povstore.uy` post-deploy (las 4 páginas con componentes migrados renderean 200, admin/orders sin Bearer responde 401 esperado). Validación funcional end-to-end (crear producto, marcar pagado, cancelar orden) requiere sesión admin manual — fuera del scope de smoke automatizable. Si en el futuro se agrega retry/refresh de token automático, basta tocar `adminFetch.ts` y todos los callers heredan el comportamiento.
- 2026-05-23: **cerrada race condition en stock** (silent oversold). Migration [migrations/20260523_apply_order_stock_strict.sql](migrations/20260523_apply_order_stock_strict.sql) re-define `apply_order_stock_once` con: (a) `PERFORM 1 FROM products ... FOR UPDATE OF p` sobre todos los products afectados ANTES de validar — serializa cualquier transacción concurrente sobre los mismos products; (b) búsqueda de shortfall en estado ya lockeado → `RAISE EXCEPTION 'insufficient_stock' USING ERRCODE='P0001'` + DETAIL estructurado (`product_id=X name=Y available=N needed=M`); (c) decremento estricto sin `GREATEST(0, …)` que silenciaba oversold. Wrapper TS [src/lib/stock/applyOrderStockOnce.ts](src/lib/stock/applyOrderStockOnce.ts) parsea el DETAIL y devuelve shape estructurado `{ok:false, reason:'insufficient_stock', shortfall:{...}}`. Callers actualizados: mp-webhook responde **200** con reason en body para insufficient_stock (no retry — stock no se arregla en segundos) + log warn estructurado; otros errores siguen 500 (retry vale la pena). admin/orders/[id] manual mark-paid devuelve **409** con `shortfall` en body para que admin vea el producto que falta. 2 mensajes nuevos en diccionarios (`apiErrorMessages.mpWebhook.insufficientStock`, `adminOrderApiMessages.insufficientStockOnApply`). Permisos del RPC corregidos siguiendo el patrón canónico (REVOKE explícito de anon/authenticated por el gotcha de Supabase default privileges — el viejo estaba abierto desde antes del descubrimiento del gotcha). Eliminada rama legacy `IF v_has_line_type` (dead code, la columna existe desde stage2a). 12 tests nuevos: 7 wrapper (parsing de DETAIL + edge cases), 2 webhook (insufficient_stock → 200 vs db error → 500), 3 admin (insufficient_stock → 409 con shortfall vs db error → 500). Suite 127/127. Smoke local 3/3 + verificación en Studio 3/3 (`pg_get_functiondef` matchea, permisos correctos, 0 productos con stock negativo — el bug nunca se gatilló históricamente, era riesgo latente no consumado). **Verificado en prod 2026-05-23**: smoke 4/4 contra `povstore.uy` post-deploy (home 200, webhook sin firma 401 missing_signature, create-order body inválido 400, admin/orders sin Bearer 401). El riesgo que el audit externo NO detectó (porque está oculto en el SQL del RPC, no en código TS visible).
- 2026-05-23: cerrado **PF-09** (delivery_method explícito). ENUM Postgres `delivery_method('delivery','pickup')` + columna NOT NULL en `orders` + RPC `create_order_transactional` actualizado con `p_delivery_method` (DEFAULT 'delivery' transitorio para no romper handler viejo durante la ventana de deploy — se puede remover después). Backfill basado en doble señal: `notes ILIKE '%Retiro en local físico%'` como primaria, `shipping_address IS NULL OR ''` como fallback. Resultado de la verificación post-migration en Studio: 17 pickup + 4 delivery clasificadas, 0 NULLs, 0 false positives de la señal fallback (todos los pickup matchearon el pattern notes). Helper centralizado [src/lib/orders/deliveryMethod.ts](src/lib/orders/deliveryMethod.ts) (`isPickup`, `isDelivery`, `PICKUP_ADDRESS`) reemplaza el patrón `!shipping_address` que estaba hardcodeado en 9 sitios (3 más de lo que CLAUDE.md catalogaba — el agente exploratorio encontró 3 `isPickup` extras en OrderDetailsModal). Archivos afectados: migration SQL + helper + tests (12 tests nuevos) + 3 SELECTs server-side (`/api/order-details`, `/api/admin/orders`, `AdminDashboard`) + 6 interfaces TypeScript + `parsePickupFromNotes` eliminado completamente de OrderConfirmation (notes deja de ser fuente de verdad para detección). Suite 115/115 (103 base + 9 helper + 3 contrato RPC PF-09). Smoke local 3/3 contra `localhost:4028`. Permisos del RPC nuevo verificados en prod: `service_role=true`, `anon/authenticated=false` (mismo patrón canónico que PF-05/PF-08). Operacionalmente: migration corrida en Supabase Studio antes del push para minimizar ventana de inconsistencia.
- 2026-05-23: **rate-limit Upstash en endpoints públicos**. Nuevo módulo [src/lib/rateLimit/](src/lib/rateLimit/) (`client.ts` singleton Redis REST, `limiters.ts` por endpoint con sliding window, `apply.ts` helper + `getClientIp`). Límites: create-order **5/min + 20/h**, mp-preference **20/min**, newsletter/subscribe **3/min + 10/día**, buckets por IP (primer `x-forwarded-for` → `x-real-ip` → `'anonymous'`). Response `429` con headers `X-RateLimit-Limit/Remaining/Reset` + `Retry-After`, mensaje en `apiErrorMessages.common.rateLimited`. **Fail-open** si Upstash es inalcanzable o si faltan env vars (`UPSTASH_REDIS_REST_URL`, `UPSTASH_REDIS_REST_TOKEN`) — no bloquea ventas por glitch infra. Deps nuevas: `@upstash/ratelimit@^2.0.8`, `@upstash/redis@^1.38.0`. 10 tests nuevos en `apply.test.ts` (suite 103/103). Smoke test local 4/4: newsletter 1-3 → 201, #4 → 429 con headers correctos, IP nueva pasa → buckets aislados, create-order #6 desde misma IP → 429. Cierra el item 🟠 "Rate-limit en endpoints públicos" de Próximos pasos. **Verificado en prod 2026-05-23**: 5/5 contra `https://povstore.uy/api/newsletter/subscribe` (3× 201 + 2× 429), headers HTTP/2 `retry-after: 33` + `x-ratelimit-limit: 3` + `x-ratelimit-remaining: 0` correctos, mp-preference no afectado por throttle de newsletter (buckets aislados por endpoint), `x-ratelimit-limit` exacto confirma Upstash respondiendo (no fail-open silencioso).
- 2026-05-24: cerrado follow-up **Tests del guard PF-07**. Nuevo [src/app/api/mp-preference/route.test.ts](src/app/api/mp-preference/route.test.ts) con 5 casos cubriendo las 3 ramas de rechazo (`payment_status='completed'` → 409 `orderAlreadyPaid`, `payment_status='refunded'` → 409 `orderRefunded`, `order_status='cancelled'` → 409 `orderCancelled`) + 2 ramas permisivas (`payment_status='failed'` retry intencional → 200 con `mp_preference_id` persistido, `pending/pending` happy path → 200). Mocks de `next/server`, `@/lib/supabaseAdmin`, `@/lib/rateLimit/apply` (bypass) y `@/lib/rateLimit/limiters` (null). Aserts referencian `apiErrorMessages.mpPreference.*` desde el diccionario (lección PF-10). Suite full 132/132 (127 antes + 5 nuevos). Cierra el follow-up "Tests del guard PF-07" listado en sección Audit PF-XX.
