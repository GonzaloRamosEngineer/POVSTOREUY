...
export default function ProductStickyNav({
  ...
  const formattedRating = useMemo(() => {
    return Number.isFinite(averageRating) ? averageRating.toFixed(1) : '5.0';
  }, [averageRating]);
      ...
      <div className="fixed top-[64px] left-0 right-0 w-full bg-white/95 backdrop-blur-md shadow-sm z-40 border-b border-gray-200 animate-in slide-in-from-top-2 duration-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="flex justify-between items-center h-14 relative">
            ...
            <div className="flex items-center gap-3 w-auto md:w-1/3 min-w-0">
              <div className="relative w-8 h-8 rounded border border-gray-100 overflow-hidden bg-white hidden sm:block shrink-0">
                <img src={productImage} alt={productName} className="w-full h-full object-contain" />
              </div>

              <div className="flex flex-col leading-tight min-w-0">
                <span className="font-bold text-gray-900 text-[11px] md:text-xs truncate max-w-[120px] md:max-w-[180px]">
                  {productName}
                </span>

                <div className="flex items-center gap-2 min-w-0">
                  <span className="font-mono text-[11px] text-red-600 font-bold shrink-0">
                    $U {productPrice.toLocaleString('es-UY')}
                  </span>

                  <div className="flex items-center gap-0.5 border-l border-gray-200 pl-2 min-w-0">
                    <Icon name="StarIcon" size={10} variant="solid" className="text-yellow-400 shrink-0" />
                    <span className="text-[10px] text-gray-400 font-bold">{formattedRating}</span>
                    {totalReviews > 0 && (
            ...
            <div className="w-auto md:w-1/3 flex justify-end">
              <button
                ...
                type="button"
              >
                <span className="hidden md:inline text-[10px] font-black uppercase tracking-widest mr-2">
                  Comprar
                </span>
                <Icon
                  name="ArrowUpIcon"
