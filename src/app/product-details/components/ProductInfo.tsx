...
interface ProductInfoProps {
  ...
  cashPrice?: number | null; // NUEVO: Precio Transferencia
  cardPrice?: number | null; // NUEVO: Precio Tarjeta
  rating: number;
  reviewCount: number;
  description: string;
  resumen?: string;
...
export default function ProductInfo({
  ...
  const scrollToReviews = () => {
    ...
    if (reviewsSection) {
      const offset = 100;
      const elementPosition = reviewsSection.getBoundingClientRect().top - document.body.getBoundingClientRect().top - offset;
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center gap-2 pb-2">
        ...
        <span className="text-sm text-gray-500 font-medium">
          {rating.toFixed(1)} • <button onClick={scrollToReviews} className="hover:text-blue-600 hover:underline transition-colors">{reviewCount} opiniones</button>
        </span>
      </div>

      {/* DISEÑO DEL RESUMEN (TAGLINE) CON LÍNEA DIVISORIA */}
      {resumen && (
      ...
      {/* --- PRECIOS REDISEÑADOS (Transferencia vs Tarjeta) --- */}
      <div className="pt-4 flex flex-col gap-2">
        {showDualPricing ? (
          <>
            {/* 1. Precio Transferencia (Destacado Verde) */}
            <div>
              <div className="flex items-center gap-3">
                <span className="text-4xl font-black text-[#10b981] tracking-tight">
            ...
            <div className="mt-3">
              ...
              {/* Precio Tachado (si existe descuento original) */}
          ...
          /* Diseño fallback (por si un producto viejo no tiene los precios nuevos cargados) */
          <>
