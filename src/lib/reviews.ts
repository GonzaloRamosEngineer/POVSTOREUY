...
function fakeDisplayName(baseName: string, seed: string) {
  const aliases = [
    'Martín R.',
    'Sofía P.',
    'Nico V.',
  ...
  const looksLikeAdmin =
    normalized === 'admin pov store' ||
    normalized === 'digitalmatch.global' ||
    normalized === 'digitalmatch global';

  if (!looksLikeAdmin) return baseName;

  const n = [...seed].reduce((acc, ch) => acc + ch.charCodeAt(0), 0);
  return aliases[n % aliases.length];
}
...
export function normalizeReviewRow(row: any): AppReview {
  ...
  return {
    ...
    author: fakeDisplayName(asString(profile?.full_name || 'Cliente POV'), asString(row?.id)),
